...
export function DashboardClient({
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar */}
      <aside className="hidden lg:flex flex-col w-56 shrink-0 border-r border-stone-200 bg-white px-3 py-6 gap-1 fixed top-12 bottom-0">
        <p className="px-3 mb-3 text-xs font-semibold uppercase tracking-wider text-stone-400">Dashboard</p>
        {TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
      ...
      {/* Mobile tab bar */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex border-t border-stone-200 bg-white">
        {TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex flex-1 flex-col items-center gap-0.5 py-2 text-xs font-medium transition ${
              tab === t.id ? 'text-stone-900' : 'text-stone-400'
            }`}
          >
      ...
      <main className="flex-1 lg:ml-56 px-4 lg:px-10 py-8 pb-24 lg:pb-8 max-w-3xl">
        <h1 className="text-xl font-bold mb-6 text-stone-900">
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-3">
            <div>
              ...
              <p className="text-xs text-stone-400 mt-0.5">Select all that apply</p>
            </div>
            <div className="flex flex-col gap-4">
              {PROFESSION_OPTIONS.map(({ group, items }) => (
                <div key={group}>
                  <p className="text-xs font-semibold uppercase tracking-wider text-stone-400 mb-2">{group}</p>
            ...
            {professions.length > 0 && (
              <p className="text-xs text-stone-500">
                Selected: <span className="font-medium text-stone-700">{professions.join(', ')}</span>
              </p>
            )}
          </div>

          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-stone-700">About</label>
            <textarea
              value={bio}
  ...
  return (
    <Card className="p-6">
      ...
      <form onSubmit={handleAdd} className="flex flex-col gap-3 mb-6">
        {/* Image drop zone */}
        <label className="flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-stone-300 bg-stone-50 cursor-pointer hover:border-stone-400 transition h-32 relative overflow-hidden">
          {imagePreview ? (
            ...
            <>
              ...
              <span className="text-xs text-stone-500">Click to upload photo (max 2 MB)</span>
            </>
          )}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-base font-semibold text-slate-800 tracking-tight">{monthLabel}</h3>
        <div className="flex items-center gap-1.5">
          {loading && (
            <span className="text-xs text-slate-400 mr-2 animate-pulse">Refreshing…</span>
          )}
          <button
            onClick={prevWeek}
            className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-800 transition-colors text-sm font-medium"
          >‹</button>
          <button
            onClick={goToday}
            className="px-3 h-8 rounded-lg text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors"
          >Today</button>
          <button
            onClick={nextWeek}
            className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-800 transition-colors text-sm font-medium"
      ...
      <div className="rounded-2xl overflow-hidden shadow-sm border border-slate-200 bg-white">
        ...
        <div className="overflow-y-auto" style={{ maxHeight: '380px' }}>
          {HOURS.slice(0, -1).map((hour) => (
            <div key={hour} className="grid border-b border-slate-100 last:border-0"
              ...
              {days.map((date, di) => {
                ...
                let cellClass = 'border-l border-slate-100 h-10 px-1 py-0.5 flex flex-col items-center justify-center transition-all duration-150 '
                ...
                if (status === 'outside') {
                  ...
                  cellClass += isToday
                    ...
                    : 'bg-white hover:bg-emerald-50 cursor-pointer group'
                  inner = (
                ...
                return (
                  ...
                  >
                    {isActing ? <span className="text-[10px] text-slate-400">…</span> : inner}
                  </button>
                )
              })}
            </div>
      ...
      <div className="flex flex-wrap gap-2">
        {[
          { bg: 'bg-white ring-1 ring-slate-200', label: 'Available' },
          ...
          { bg: 'bg-slate-50 ring-1 ring-slate-100', label: 'Outside hours' },
        ].map(({ bg, label }) => (
